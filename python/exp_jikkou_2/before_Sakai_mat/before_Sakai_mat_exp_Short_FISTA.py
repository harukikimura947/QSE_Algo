...
#                 Z_J[i][j * self.prm.K + i] = 1.0
        ...
        self.sparse_I = spsp.coo_matrix(Z_I)
        self.sparse_J = spsp.coo_matrix(Z_J)

        # self.sparse_I_alpha_S_R = self.alpha_S_R * self.sparse_I.T
        # self.sparse_J_alpha_S_W = self.alpha_S_W * self.sparse_J.T

        # self.sparse_I_alpha_L_R = self.alpha_L_R * self.sparse_I.T
        # self.sparse_J_alpha_L_W = self.alpha_L_W * self.sparse_J.T

        # self.one_K_alpha_S_R = np.ones(self.prm.K) * self.alpha_S_R
        # self.one_K_alpha_S_W = np.ones(self.prm.K) * self.alpha_S_W
        # self.one_K_alpha_L_R = np.ones(self.prm.K) * self.alpha_L_R
        # self.one_K_alpha_L_W = np.ones(self.prm.K) * self.alpha_L_W
    ...
    def demand_sparse(self, R, W):
        ...
        #案1：演算用のsparse行列を用いる
        # S_H = self.T_power\
        #     * (self.sparse_I_alpha_S_R @ ((R_inv) ** self.power_S_H_R))\
        #     * (self.sparse_J_alpha_S_W @ ((W_inv) ** self.power_S_H_W))
        
        # L_H = self.T_power\
        #     * (self.sparse_I_alpha_L_R @ ((R_inv) ** self.power_L_H_R))\
        #     * (self.sparse_J_alpha_L_W @ ((W_inv) ** self.power_L_H_W))

        # 外積案 np.outer
        S_H = self.T_power\
            * np.outer((self.alpha_S_R * R_inv ** self.power_S_H_R), (self.alpha_S_W * W_inv ** self.power_S_H_W))
        
        L_H = self.T_power\
            * np.outer((self.alpha_L_R * R_inv ** self.power_L_H_R), (self.alpha_L_W * W_inv ** self.power_L_H_W))

        #案2：numpyを用いて，価格ベクトルを整形する
        # S_H = self.T_power\
        #     * np.repeat((self.alpha_S_R * 1.0/(R ** self.power_S_H_R)), self.prm.K)\
        #     * np.tile((self.alpha_S_W * 1.0/(W ** self.power_S_H_W)), self.prm.K)
        
        # L_H = self.T_power\
        #     * np.repeat((self.alpha_L_R * 1.0/(R ** self.power_L_H_R)), self.prm.K)\
        #     * np.tile((self.alpha_L_W * 1.0/(W ** self.power_L_H_W)), self.prm.K)
    ...
    def pi(self, R, W, m):
        ...
        pi = np.dot(self.prm.D, m)\
           ...
           * (self.beta_L_R * R_inv ** self.power_L_F_R)\
           * (self.beta_S_W * W_inv ** self.power_S_F_W)
    ...
    def v(self, R, W):
        ...
        v = self.prm.E * W \
          ...
          * self.T_power\
          * np.outer((self.alpha_L_R * R_inv ** self.power_L_H_R), (self.alpha_S_W * W_inv ** self.power_S_H_W))\
    ...
    def short_solve(self, RW_ini, m_fixed, n_fixed, err_short, short_itr, rel):
        ...
        for k in range(1, short_itr):
            ...
            p_bar = np.maximum(self.prm.RW_proj, RW + ((t_before - 1) / t) * (RW - RW_before))
