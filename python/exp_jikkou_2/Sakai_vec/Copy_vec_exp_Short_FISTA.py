...
#                 Z_J[i][j * self.prm.K + i] = 1.0
    ...
    def demand_sparse(self, R, W):
        ...
        # 外積案 np.outer
        S_H = self.T_power\
            * np.outer((self.alpha_S_R * R_inv ** self.power_S_H_R), (self.alpha_S_W * W_inv ** self.power_S_H_W)).ravel()
        
        L_H = self.T_power\
            * np.outer((self.alpha_L_R * R_inv ** self.power_L_H_R), (self.alpha_L_W * W_inv ** self.power_L_H_W)).ravel()
