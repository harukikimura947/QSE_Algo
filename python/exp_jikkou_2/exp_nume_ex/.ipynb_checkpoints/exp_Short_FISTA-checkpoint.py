...
class Short:
    ...
    def __init__(self, prm, algprm):
        ...
        self.prm = prm
        self.algprm = algprm
        # self.btra = btra.btra(prm, algprm)
    ...
    def demand_res(self, R, W):
        ...
        S_H = self.T_power\
            * (self.alpha_S_R * (1.0 / R) ** self.power_S_R)[:, np.newaxis]\
            * (self.alpha_S_W * (1.0 / W) ** self.power_S_W)
        
        L_H = self.T_power\
            * (self.alpha_L_R * (1.0 / R) ** self.power_L_R)[:, np.newaxis]\
            * (self.alpha_L_W * (1.0 / W) ** self.power_L_W)
    ...
    def demand(self, R, W):
        ...
        W: numpy.ndarray(matrix) (K, )
            賃金リスト
        '''
        #同じような形が何回も現れているので，それは一番最初に求めておく．べき乗はかなり重いからあまりやりたくない！
    ...
    def demand_hat(self, R, W):
        ...
        S_H_hat = ((1 / self.prm.T) ** (1 / (1 - self.prm.alpha_1 - self.prm.alpha_2))) \
                * ((self.prm.alpha_2 / W) ** (self.prm.alpha_2 / (1 - self.prm.alpha_1 - self.prm.alpha_2)))
        
        L_H_hat = ((1 / self.prm.T) ** (1 / (1 - self.prm.alpha_1 - self.prm.alpha_2))) \
                * ((self.prm.alpha_1 / R) ** (self.prm.alpha_1 / (1 - self.prm.alpha_1 - self.prm.alpha_2)))[:, np.newaxis]
        ...
        return S_H_hat, L_H_hat, S_F, L_F
...
#         S_H_hat, L_H_hat, S_F, L_F = self.demand_hat(R, W)
        
#         R_arr = (self.prm.alpha_1 / R) ** ((1 - self.prm.alpha_2) / (1 - self.prm.alpha_1 - self.prm.alpha_2))
#         W_arr = (self.prm.alpha_2 / W) ** ((1 - self.prm.alpha_1) / (1 - self.prm.alpha_1 - self.prm.alpha_2))
        
#         dR = self.prm.S_bar - R_arr * np.sum(S_H_hat * n, axis=1) - S_F * m  # forall i
#         dW = self.prm.E * np.sum(n, axis=0) - W_arr * np.sum(L_H_hat * n, axis=0) - L_F * m # forall j
