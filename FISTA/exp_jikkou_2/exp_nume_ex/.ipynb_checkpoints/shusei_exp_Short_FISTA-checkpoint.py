...
class Short:
    ...
    def demand_res(self, R, W):
        ...
        S_H = self.T_power\
            * (self.alpha_S_R * (1.0 / R) ** self.power_S_H_R)[:, np.newaxis]\
            * (self.alpha_S_W * (1.0 / W) ** self.power_S_H_W)
        
        L_H = self.T_power\
            * (self.alpha_L_R * (1.0 / R) ** self.power_L_H_R)[:, np.newaxis]\
            * (self.alpha_L_W * (1.0 / W) ** self.power_L_H_W)
    ...
    def Z_SD(self, RW, m, n):
        ...
        #flatten()遅い！！
        F_value = self.v(R, W).flatten() @ n.flatten()\
...
#         dW = self.prm.E * np.sum(n, axis=0) - W_arr * np.sum(L_H_hat * n, axis=0) - L_F * m # forall j
        ...
        S_H, L_H, S_F, L_F = self.demand_res(R, W)
