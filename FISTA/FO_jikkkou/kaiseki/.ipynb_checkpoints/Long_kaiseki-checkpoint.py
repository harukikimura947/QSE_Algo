...
class Long:
    ...
    def solve(self, m0, n0, long_itr, err_long):
        ...
        for k in range(1, long_itr):
            ...
            m, n = self.breakingdown(mn_hist[k-1])
            
            #Step.1 短期均衡を解く
            R_true = ((self.prm.alpha_1 * np.sum(n, axis=1) + self.prm.beta_1 * m) / self.prm.S_bar)
            W_true = (1 / self.prm.E) * (self.prm.beta_2 * (m / np.sum(n, axis=0)) + self.prm.alpha_2)
            ...
            pi_true = np.maximum(0.1, np.dot(self.prm.D, m)\
            ...
            #Step.2 探索方向を決める
            mn_d_hist[k] = self.logit(pi_true, v_true)
            
            #Step.3 ステップサイズを決める
            alpha = 0.1
            
            #Step.4 解を更新する
            mn_hist[k] = (1 - alpha) * mn_hist[k-1] + alpha * mn_d_hist[k]
            
            max_value = k
            
            #Step.5 収束判定
            if np.max(abs((mn_hist[k] - mn_hist[k-1]) / mn_hist[k-1])) < err_long:
                break
        
        print("long_max_value:", max_value)
        
        m_true, n_true = self.breakingdown(mn_hist[max_value])
        
        G = np.sum(pi_true * m_true) + np.sum(v_true * n_true)\
        - (self.prm.M * np.log(np.sum(np.exp(self.prm.theta_firm * pi_true))) / self.prm.theta_firm) - (self.prm.N * np.log(np.sum(np.exp(self.prm.theta_house * v_true))) / self.prm.theta_house) \
        - (np.sum(m_true * np.log(m_true / self.prm.M)) / self.prm.theta_firm) - (np.sum(n_true * np.log(n_true / self.prm.N)) / self.prm.theta_house)
