...
def demand():
    ...
    S_H = ((1 / np.maximum(prm.alter_T, prm.T)) ** (1 / (1 - prm.alpha_1 - prm.alpha_2))) \
        * (((prm.alpha_1 / R) * np.ones((prm.K, prm.K))) ** ((1 - prm.alpha_2) / (1 - prm.alpha_1 - prm.alpha_2))).T \
        * (((prm.alpha_2 / W) * np.ones((prm.K, prm.K))) ** (prm.alpha_2 / (1 - prm.alpha_1 - prm.alpha_2)))

    L_H = ((1 / np.maximum(prm.alter_T, prm.T)) ** (1 / (1 -prm.alpha_1 - prm.alpha_2))) \
        * (((prm.alpha_1 / R) * np.ones((prm.K, prm.K))) ** (prm.alpha_1 / (1 - prm.alpha_1 - prm.alpha_2))).T \
        * (((prm.alpha_2 / W) * np.ones((prm.K, prm.K))) ** ((1 - prm.alpha_1) / (1 - prm.alpha_1 - prm.alpha_2)))

    S_F = (prm.beta_1 / R) ** ((1 - prm.beta_2) / (1 - prm.beta_1 - prm.beta_2)) *\
          (prm.beta_2 / W) ** (prm.beta_2 / (1 - prm.beta_1 - prm.beta_2)) #(K, )

    L_F = (prm.beta_1 / R) ** (prm.beta_1 / (1 - prm.beta_1 - prm.beta_2)) *\
          (prm.beta_2 / W) ** ((1 - prm.beta_1) / (1 - prm.beta_1 - prm.beta_2)) #(K, )
