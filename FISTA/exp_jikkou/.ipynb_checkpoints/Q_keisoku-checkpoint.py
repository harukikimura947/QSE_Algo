...
def Q():
    ...
    L_bar = (algprm.eta ** i) * L
    p_bar = np.concatenate((R, W))
    p = np.maximum(algprm.p_proj, p_bar - (self.short_dual_df(p_bar, m_fixed, n_fixed) / L_bar))

    Q = short.Z_SD(p_bar, m_fixed, n_fixed) + np.dot(p - p_bar, short.short_dual_df(p_bar, m_fixed, n_fixed)) + (L_bar / 2) * np.linalg.norm(p - p_bar) ** 2
