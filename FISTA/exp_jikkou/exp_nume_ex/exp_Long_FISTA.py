...
class Long:
    ...
    def solve(self, m0, n0, RW_ini, err_short, err_long, obj_corr, long_itr):
        ...
        for k in range(1, long_itr):
            ...
            self.short.short_solve(RW_ini, m_before, n_before, err_short, obj_corr = 1.0,
                                   short_itr=100000, rel=0)
            
            if k % 10 == 1:
                S_H_hat, L_H_hat, S_F, L_F = self.short.demand_hat(R, W)
                
                vmin = 0
                vmax = self.prm.S_bar
                
                mat = np.reshape(m_before * S_F, (self.prm.Col, self.prm.Col))
            ...
            #Step.2 探索方向を決める
            mn_d = self.logit(R, W, m_before)
            
            #Step.3 解を更新する
            obj = self.Z_LP(mn_before, RW)
            
            long_iteration.append(k)
            obj_list.append(obj)
